var DeviceConfiguration = (function() {
  ...
  var sectionsVisible = {
    //All false because the application's start state is the search screen.  onHistory will restore them.
    tools: false,
    slides: false,
    header: false,
    keyboard: false,
  ...
  var setSectionVisibility = function(section, visible) {
    if (allowShowingChrome) {
      if (
        (allowedToHideHeader || section != "header") &&
  ...
  var setDefaultOptions = function() {
    ...
    try {
      if (
        ...
        "jid" in Conversations.getCurrentConversation()
      ) {
  ...
  var defaultFitFunction = function() {
    customizableFitFunction(
      sectionsVisible.header,
      sectionsVisible.tools,
      sectionsVisible.slides,
      sectionsVisible.keyboard
    );
  };
  var fitFunction = defaultFitFunction;
  var projectorFitFunction = function() {
    customizableFitFunction(false, false, false, false);
  };
    ...
    return {
      x: _.sum(
        _.map(items, function(item) {
          var l =
            dropPx(item.css("margin-left")) +
            dropPx(item.css("padding-left")) +
            dropPx(item.css("border"));
          var r =
            dropPx(item.css("margin-right")) +
            dropPx(item.css("padding-right")) +
            dropPx(item.css("border"));
          return l + r;
        })
      ),
      y: _.sum(
        _.map(items, function(item) {
          var t =
            dropPx(item.css("margin-top")) +
            dropPx(item.css("padding-top")) +
            dropPx(item.css("border"));
          var b =
            dropPx(item.css("margin-bottom")) +
            dropPx(item.css("padding-bottom")) +
            dropPx(item.css("border"));
          return t + b;
        })
      )
  ...
  var customizableFitFunction = function(
    _showHeader,
    _showTools,
    _showSlides,
    _showKeyboard
    ...
    if (typeof boardContext != "undefined") {
      ...
      var performRemeasure = function() {
        ...
        if (Modes.currentMode == Modes.select) {
          Modes.select.updateAdministerContentVisualState(
        ...
        if (flexDirection == "row") {
          bwidth =
            width -
            (showTools ? toolsColumn.width() : 0) -
            (showSlides ? thumbsColumn.width() : 0) -
            marginsFor([toolsColumn, thumbsColumn, boardColumn]).x -
            gutterWidth;
          bheight =
            height -
            headerHeight -
            footerHeight -
            marginsFor([masterHeader, masterFooter, boardColumn]).y;
        } else {
          ...
          if (showKeyboard) {
            var keyboardSize =
              currentDevice == "iPad"
                ? DeviceConfiguration.preferredSizes.keyboard.iphone
                : DeviceConfiguration.preferredSizes.keyboard.ipad;
            bheight -= bwidth;
        ...
        var messageAdorner = comp("#messageAdorner");
        masterFooter
          .width(
            width -
              thumbsColumn.width() -
          ...
          .css({
            "margin-left": sprintf(
              "%spx",
              toolsColumn.width() + gutterWidth * 4.5
            )
          });
        board.width(bwidth);
