...
function registerPositionHandlers(contexts, down, move, up) {
      ...
      _.each(category, function(interactable) {
        if (interactable != undefined && event in interactable) {
          if (
            ...
            intersectRect(worldRay, interactable.getBounds())
          ) {
  ...
  $.each(contexts, function(i, _context) {
    ...
    var checkIsGesture = function(pointerEvent) {
      if (
        pointerEvent !== undefined &&
        pointerEvent.originalEvent.pointerType == "touch"
      ) {
...
function aspectConstrainedRect(rect, hAlign, vAlign) {
  ...
  return {
    ...
    up: function(worldPos) {
      ...
      if (position < bw) {
        ...
        var seekPos =
          ((position - bw) / (video.width - 2 * bw)) * mediaState.duration;
        video.seek(seekPos);
      }
      return false;
    },
    ...
    render: function(canvasContext) {
      if (video.identity in boardContent.videos) {
        ...
        bounds = [x, video.bounds[3], video.bounds[2], video.bounds[3] + h];
        ...
        var progressWidth =
          (mediaState.currentTime / mediaState.duration) * progressWidth;
        canvasContext.fillRect(progressX, tl.y, progressWidth, height);
...
var Modes = (function() {
  ...
  $(function() {
    ...
    var tween = new TWEEN.Tween(attrs)
      .delay(fadeInInterval)
      .to({ opacity: targetAlpha }, fadeInInterval);
    var tick = function() {
      TWEEN.update();
      blit();
    ...
    var clipToInteractableSpace = function(y) {
      ...
      var clipped = Math.min(
        viewboxY + viewboxHeight - (floor - s / 2),
        Math.max(y, viewboxY + ceiling)
      );
      return clipped;
    };
      ...
      return {
        ...
        rehome: function(root) {
          if (!manualMove.activated) {
            ...
            bounds = [center - s / 2, y - s, center + s / 2, y];
          }
        },
        down: function(worldPos) {
    ...
    var resizeAspectLocked = (function() {
      ...
      return {
        ...
        move: function(worldPos) {
          if (resizeAspectLocked.activated) {
            bounds = [worldPos.x - s, bounds[1], worldPos.x + s, bounds[3]];
  ...
  return {
    ...
    text: (function() {
      ...
      var toggleFormattingProperty = function(prop) {
        return function() {
          ...
          if (!matched) {
            carota.runs.nextInsertFormatting =
              carota.runs.nextInsertFormatting || {};
            var intention = (carota.runs.nextInsertFormatting[prop] = !carota
              .runs.nextInsertFormatting[prop]);
      ...
      return {
        ...
        scrollToCursor: function(editor) {
          ...
          var linesInBox = Math.floor(
            scaleScreenToWorld(boardContext.height) / cursorY.h
          );
          if (DeviceConfiguration.hasOnScreenKeyboard()) {
            var scrollOffset =
              Math.min(linesFromTop, Math.max(0, linesInBox - 2)) * cursorY.h;
            ...
            var margin = 8;
            var freeLinesFromBoxTop =
              Math.floor((viewboxHeight - boxOffset) / cursorY.h) - margin;
            var takenLinesFromBoxTop = Math.floor(cursorY.t / cursorY.h);
            var adjustment =
              Math.max(0, takenLinesFromBoxTop - freeLinesFromBoxTop) *
              cursorY.h;
            if (adjustment != 0) {
              TweenController.zoomAndPanViewbox(
                viewboxX,
          ...
          if (!editor.doc) {
            ...
            if (isAuthor) {
              ...
              editor.doc.contentChanged(function() {
                //This one isn't debounced so it scrolls as we type and stays up to date
                Modes.text.scrollToCursor(editor);
                blit();
              });
              editor.doc.selectionChanged(function(
                formatReport,
                canMoveViewport
              ) {
        ...
        editorAt: function(x, y, z, worldPos) {
          ...
          var texts = _.values(boardContent.multiWordTexts)
        ...
        activate: function() {
          ...
          var down = function(x, y, z, worldPos) {
            ...
            if (editor) {
              ...
              editor.mousedownHandler(
                Modes.text.contextFor(editor, worldPos).node
              );
            }
          };
          ...
          var up = function(x, y, z, worldPos) {
            ...
            _.each(boardContent.multiWordTexts, function(t) {
              ...
              if (
                t.doc.selection.start + t.doc.selection.end > 0 &&
                t.doc.identity != editor.identity
              ) {
                t.doc.select(0, 0);
                t.doc.updateCanvas();
              }
            ...
            if (oldEditor) {
              ...
              var newEditor = createBlankText(
                { x: oldEditor.x, y: oldEditor.y },
                [
                  {
                    text: oldEditor.text,
              ...
              sel = { multiWordTexts: {} };
              sel.multiWordTexts[newEditor.identity] =
                boardContent.multiWordTexts[newEditor.identity];
              ...
              var newEditor = createBlankText(worldPos, [
                {
                  ...
                  color:
                    carota.runs.nextInsertFormatting.color ||
                    carota.runs.defaultFormatting.color,
    ...
    video: (function() {
      ...
      var clientSideProcessVideo = function(onComplete) {
        if (
          ...
          currentVideo.fileUpload == undefined ||
            ...
            success: function(e) {
              ...
              var videoStanza = {
                ...
                bounds: [
                  currentVideo.x,
                  currentVideo.y,
                  currentVideo.x + currentVideo.width,
                  currentVideo.y + currentVideo.height
                ],
                ...
                x: currentVideo.x,
                y: currentVideo.y,
                audiences: _.map(Conversations.getCurrentGroup(), "id")
                  .concat(ContentFilter.getAudiences())
                  .map(audienceToStanza)
              };
              registerTracker(newIdentity, function() {
                ...
                Modes.select.clearSelection();
                Modes.select.selected.videos[videoStanza.identity] =
                  boardContent.videos[videoStanza.identity];
                Progress.call("onSelectionChanged", [Modes.select.selected]);
              });
      ...
      $(function() {
        if (!hasInitialized) {
          hasInitialized = true;
          insertOptions = $("#videoInsertOptions").css({
            position: "absolute",
            left: 0,
            top: 0
          });
          insertOptionsClose = $("#videoInsertOptionsClose").click(
            Modes.select.activate
          );
          videoFileChoice = $("#videoFileChoice").attr("accept", "video/mp4");
          videoFileChoice[0].addEventListener(
            "change",
            function(e) {
    ...
    image: (function() {
      ...
      var imageModes = (function() {
        ...
        var modes = {
          native: {
            resizeFunc: function(w, h) {
              return { w: w, h: h, q: 1.0 };
            },
            selector: "#imageInsertNative"
          },
          optimized: {
            resizeFunc: function(w, h) {
              return keepUnder(1 * megaPixels, w, h, 0.4);
            },
            selector: "#imageInsertOptimized"
          },
          highDef: {
            resizeFunc: function(w, h) {
              return keepUnder(3 * megaPixels, w, h, 0.8);
            },
      ...
      var clientSideProcessImage = function(onComplete, thisCurrentImage) {
        var state =
          thisCurrentImage == undefined ? currentImage : thisCurrentImage;
        if (
          state == undefined ||
        ...
        reader.onload = function(readerE) {
          var originalSrc = readerE.target.result;
          clientSideProcessImageSrc(originalSrc, state, onComplete, function(
            img
          ) {
            var originalSize = originalSrc.length;
      ...
      var clientSideProcessImageSrc = function(
        originalSrc,
        state,
        onComplete,
        ifBiggerPred
        ...
        img.onerror = function(e) {
          errorAlert(
            "Error dropping image",
            "The source server you're dragging the image from does not allow dragging the image directly.  You may need to download the image first and then upload it."
          );
        };
        img.onload = function(e) {
          ...
          thisCurrentImage.resizedImage = resizedCanvas.toDataURL(
            "image/jpeg",
            quality
          );
            ...
            success: function(e) {
              ...
              var imageStanza = {
                ...
                bounds: [
                  imageDef.x,
                  imageDef.y,
                  imageDef.x + imageDef.width,
                  imageDef.y + imageDef.height
                ],
              ...
              registerTracker(newIdentity, function() {
                ...
                Modes.select.clearSelection();
                Modes.select.selected.images[imageStanza.identity] =
                  boardContent.images[imageStanza.identity];
                Progress.call("onSelectionChanged", [Modes.select.selected]);
              });
      ...
      $(function() {
        if (!hasInitialized) {
          hasInitialized = true;
          insertOptions = $("#imageInsertOptions").css({
            position: "absolute",
            left: 0,
            top: 0
          });
          insertOptionsClose = $("#imageInsertOptionsClose").click(
            Modes.select.activate
          );
      ...
      return {
        ...
        handleDrop: function(dataTransfer, x, y) {
          ...
          _.forEach(dataTransfer.files, function(f) {
            processFile(f, "file");
          });
    ...
    select: (function() {
      ...
      Progress.conversationDetailsReceived["ModesSelect"] = function(
        ...
        if (
          isAdministeringContent &&
          !Conversations.shouldModifyConversation(conversation)
        ) {
        ...
        if (Conversations.shouldModifyConversation(conversation)) {
          $("#administerContent")
            .show()
            .unbind("click")
            .bind("click", administerContentFunction);
          $("#ban")
            .unbind("click")
            .show()
            .bind("click", banContentFunction);
        } else {
          $("#administerContent")
            .hide()
            .unbind("click");
      ...
      return {
        name: "select",
        isAdministeringContent: function() {
          return isAdministeringContent;
        },
        ...
        totalSelectedBounds: Bench.track(
          "Modes.select.totalSelectedBounds",
          function() {
            var totalBounds = {
              x: Infinity,
              y: Infinity,
        ...
        activate: function() {
          ...
          var down = function(x, y, z, worldPos, modifiers) {
            ...
            if (!modifiers.ctrl) {
              ...
              if (tb.x != Infinity) {
                ...
                Modes.select.dragging = _.some(
                  ["images", "texts", "inks", "multiWordTexts", "videos"],
                  isDragHandle
                );
              }
            }
          ...
          var up = function(x, y, z, worldPos, modifiers) {
            ...
            try {
              ...
              if (Modes.select.dragging) {
                var root = Modes.select.totalSelectedBounds();
                _.each(Modes.select.selected.multiWordTexts, function(
                  text,
                  id
                ) {
                  Modes.text.echoesToDisregard[id] = true;
                ...
                var selectionRect = rectFromTwoPoints(
                  Modes.select.marqueeWorldOrigin,
                  worldPos,
                  2
                );
                var selectionBounds = [
                ...
                var toggleCategory = function(category) {
                  $.each(intersected[category], function(id, item) {
                    ...
                    if (
                      category in Modes.select.selected &&
                      id in Modes.select.selected[category]
                    ) {
                ...
                if (selectionRect.width <= 2 && selectionRect.height <= 2) {
                  ...
                  if (hasValue(topNormalInk)) {
                    ...
                    var topMultiWordText = getMostRecentStanza(
                      intersected.multiWordTexts,
                      "multiWordText"
                    );
                    if (hasValue(topMultiWordText)) {
                      ...
                      var topText = getMostRecentStanza(
                        intersected.texts,
                        "text"
                      );
                      if (hasValue(topText)) {
                        ...
                        var highlighters = _.filter(intersected.inks, function(
                          ink
                        ) {
                          return ink.isHighlighter;
                        });
                        var topHighlighter = getMostRecentStanza(
                          highlighters,
                          "highlighter"
                        );
                        if (hasValue(topHighlighter)) {
                          ...
                          var topVideo = getMostRecentStanza(
                            intersected.videos,
                            "video"
                          );
                          if (hasValue(topVideo)) {
                            ...
                            var topImage = getMostRecentStanza(
                              intersected.images,
                              "image"
                            );
    ...
    draw: (function() {
      ...
      var brushes = _.map(originalBrushes, function(i) {
        return _.clone(i);
      });
      var currentBrush;
      var erasing = false;
      ...
      $(function() {
        ...
        var drawTools = function() {
          ...
          _.each(container.find(".modeSpecificTool.pen"), function(button, i) {
            ...
            var thisButton = $(button)
              ...
              .addClass(brush.isHighlighter ? "fa-circle" : "fa-tint");
          ...
          if (erasing || modifiers.eraser) {
            ...
            var deleteInRay = function(coll) {
              $.each(coll, function(i, item) {
                if (
                  item.author == UserSettings.getUsername() &&
            ...
            boardContext.stroke();
            currentStroke = currentStroke.concat([
              worldPos.x,
              worldPos.y,
              mousePressure * z
            ]);
        ...
        up = function(x, y, z, worldPos, modifiers) {
          ...
          if (erasing || modifiers.eraser) {
            var deleteTransform = batchTransform();
            deleteTransform.isDeleted = true;
            deleteTransform.inkIds = _.map(deleted, function(stanza) {
      ...
      return {
        ...
        activate: function() {
          ...
          if (erasing) {
            ...
            _.each($("#drawTools").find(".modeSpecificTool.pen"), function(
              button,
              i
            ) {
