var board = function(options) {
  var getPref = function(name, defaultValue) {
    return options != undefined && name in options
      ? options[name]
      : defaultValue;
  };
  ...
  var scaleScreenToWorld = function(i) {
    ...
    var scale;
    if (p > 1) {
      //Viewbox wider than board
      scale = viewboxWidth / boardWidth;
    } else {
      //Viewbox narrower than board
  ...
  var Colors = (function() {
    ...
    var get8BitHexFromString = function(string, position) {
      return parseInt(
        string.substring(position, position + 2).toUpperCase(),
        16
      );
    };
    ...
    var getNameForColorFunction = function(color) {
      var rgb = color[0];
      var opacity = color[1];
      var lookedUpName = _.find(colors, function(c) {
        return c[1] == rgb && c[2] == opacity;
      });
      if (lookedUpName) {
  ...
  var Fonts = (function() {
    var families = ["sans-serif", "serif"];
    var sizes = [12, 14, 16, 18, 20, 30, 42, 60];
    return {
      getAllFamilies: function() {
        return _.map(families, function(f) {
          return f;
        });
      },
      getAllSizes: function() {
        return _.map(sizes, function(s) {
          return s;
        });
      }
    };
  ...
  var Brushes = (function() {
    ...
    return {
      getDefaultBrushes: function() {
        return _.map(brushes, function(b) {
          return b;
        });
      },
      getAllBrushSizes: function() {
        return _.map(brushSizes, function(b) {
          return b;
        });
      }
    };
 ...
 * */
  ...
  var detectPointerEvents = function() {
    try {
      return (
        ("pointerEnabled" in Navigator && Navigator.pointerEnabled == true) ||
  ...
  var registerPositionHandlers = function(contexts, down, move, up) {
    ...
    $.each(contexts, function(i, _context) {
      ...
      var updatePoint = function(pointerEvent) {
        ...
        var isEraser =
          pointerEvent.originalEvent.pointerType == "pen" &&
          pointerEvent.originalEvent.button == 5;
        var o = offset();
      ...
      if (detectPointerEvents()) {
        var performGesture = function() {
          if (_.size(trackedTouches) > 1) {
            ...
            var calculationPoints = _.map(
              _.filter(trackedTouches, function(item) {
                return _.size(item.points) > 0;
              }),
              function(item) {
            ...
            trackedTouches = {};
            var xDelta = _.meanBy(calculationPoints, function(i) {
              return i[0].x - i[1].x;
            });
            var yDelta = _.meanBy(calculationPoints, function(i) {
              return i[0].y - i[1].y;
            });

            Pan.translate(
              scaleWorldToScreen(xDelta),
              scaleWorldToScreen(yDelta)
            );

            var prevSouthMost = _.min(
              _.map(calculationPoints, function(touch) {
                return touch[0].y;
              })
            );
            var prevNorthMost = _.max(
              _.map(calculationPoints, function(touch) {
                return touch[0].y;
              })
            );
            var prevEastMost = _.min(
              _.map(calculationPoints, function(touch) {
                return touch[0].x;
              })
            );
            var prevWestMost = _.max(
              _.map(calculationPoints, function(touch) {
                return touch[0].x;
              })
            );
        ...
        context.bind("pointermove", function(e) {
          ...
          e.preventDefault();
          if (
            e.originalEvent.pointerType == e.POINTER_TYPE_TOUCH ||
            (e.originalEvent.pointerType == "touch" &&
              _.size(trackedTouches) > 1)
          ) {
            performGesture();
          }
          if (_.size(trackedTouches) == 1 && !isGesture) {
  ...
  var aspectConstrainedRect = function(rect, hAlign, vAlign) {
    // vAlign and hAlign are strings to determine how to align the position of the aspect constrained rect within itself after adjusting the proportion.  It should be "top","bottom","center" and "left","right","center".
    var proportion = boardHeight / boardWidth;
  ...
  var overlapRect = function(r1, r2) {
    ...
    return (
      (Math.max(r1[0], r2[0]) - Math.min(r1[2], r2[2])) *
  ...
  var unregisterPositionHandlers = function(context) {
    $.each(
      "pointerdown pointermove pointerup pointerout pointerleave pointercancel mouseup mousemove mousedown touchstart touchmove touchend touchcancelled mouseout touchleave gesturechange gesturestart".split(
        " "
      ),
      function(i, evt) {
        context.unbind(evt);
  ...
  var WorkQueue = (function() {
    ...
    var gracefullyResumeFunction = function() {
      stopResume();
      gracefullyResumeTimeout = setTimeout(function() {
        canWorkFunction(true);
      }, gracefullyResumeDelay);
    };
    return {
      pause: pauseFunction,
      gracefullyResume: gracefullyResumeFunction,
      enqueue: function(func) {
        //A function returning a bool, blit needed or not.
        if (isAbleToWork) {
          if (func()) {
            blit();
          }
  ...
  var Pan = {
    ...
    translate: function(xDelta, yDelta) {
      ...
      /*
         ...
         blit();
         */
      TweenController.translateViewboxRelative(
        xDelta * xScale,
        yDelta * yScale
      );
    }
  ...
  var Zoom = (function() {
    ...
    return {
      scale: function(scale, ignoreLimits) {
        ...
        TweenController.scaleAndTranslateViewbox(
          finalX,
          finalY,
          requestedWidth,
          requestedHeight
      ...
      zoom: function(scale, ignoreLimits, onComplete) {
        ...
        TweenController.zoomAndPanViewboxRelative(
          xDelta,
          yDelta,
          wDelta,
          hDelta,
  ...
  var TweenController = (function() {
    ...
    var instantAlterViewboxFunction = function(
      finalX,
      finalY,
      finalWidth,
      finalHeight,
      onComplete,
      shouldAvoidUpdatingRequestedViewbox
    ) {
      if (
        isNaN(finalX) ||
        isNaN(finalY) ||
    ...
    var tween;
    var easingAlterViewboxFunction = function(
      finalX,
      finalY,
      finalWidth,
      finalHeight,
      ...
      var hasChanged = function() {
        return (
          ...
          finalWidth > 0 &&
          finalHeight > 0 &&
          (xDelta != 0 || yDelta != 0 || widthDelta != 0 || heightDelta != 0)
        );
      };
      if (tween) {
        tween.stop();
    ...
    var panViewboxFunction = function(
      xDelta,
      yDelta,
      onComplete,
      shouldAvoidUpdatingRequestedViewbox
    ) {
      return easingAlterViewboxFunction(
        xDelta,
        yDelta,
        viewboxWidth,
        viewboxHeight,
    ...
    var translateViewboxFunction = function(
      xDelta,
      yDelta,
      onComplete,
      shouldAvoidUpdatingRequestedViewbox
    ) {
      return instantAlterViewboxFunction(
        xDelta,
        yDelta,
        viewboxWidth,
        viewboxHeight,
    ...
    var panViewboxRelativeFunction = function(
      xDelta,
      yDelta,
      onComplete,
      shouldAvoidUpdatingRequestedViewbox
    ...
    var translateViewboxRelativeFunction = function(
      xDelta,
      yDelta,
      onComplete,
      shouldAvoidUpdatingRequestedViewbox
    ...
    var zoomAndPanViewboxFunction = function(
      xDelta,
      yDelta,
      widthDelta,
      heightDelta,
    ...
    var zoomAndPanViewboxRelativeFunction = function(
      xDelta,
      yDelta,
      widthDelta,
      heightDelta,
      ...
      return easingAlterViewboxFunction(
        xDelta + viewboxX,
        yDelta + viewboxY,
        widthDelta + viewboxWidth,
        heightDelta + viewboxHeight,
        onComplete,
        shouldAvoidUpdatingRequestedViewbox
      );
    };
    var scaleAndTranslateViewboxFunction = function(
      xDelta,
      yDelta,
      widthDelta,
      heightDelta,
    ...
    var scaleAndTranslateViewboxRelativeFunction = function(
      xDelta,
      yDelta,
      widthDelta,
      heightDelta,
  ...
  var Extend = (function() {
    ...
    return {
      ...
      center: function(x, y, onComplete) {
        ...
        TweenController.panViewboxRelative(
          targetX - viewboxX,
          targetY - viewboxY,
          onComplete
        );
  ...
  var historyReceivedFunction = function(json) {
    try {
      ...
      var historyDownloadedMark,
        prerenderInkMark,
        prerenderImageMark,
        prerenderHighlightersMark,
        prerenderTextMark,
        imagesLoadedMark,
        renderMultiWordMark,
        historyDecoratorsMark,
        blitMark;
  ...
  var prerenderInk = function(ink) {
    ...
    var rawWidth =
      ink.bounds[2] - ink.bounds[0] + ink.thickness + privacyOffset * 2;
    var rawHeight =
      ink.bounds[3] - ink.bounds[1] + ink.thickness + privacyOffset * 2;
    ...
    var contentOffsetX =
      -1 *
      (ink.minX - ink.thickness / 2 - privacyOffset) *
      scaleMeasurements.scaleX;
    var contentOffsetY =
      -1 *
      (ink.minY - ink.thickness / 2 - privacyOffset) *
      scaleMeasurements.scaleY;
    var x, y, pr, newPr, p;
    if (isPrivate) {
  ...
  var alertCanvas = function(label) {
    ...
    window.open(
      url,
      label,
      sprintf("width=%s, height=%s", canvas.width, canvas.height)
    );
  };
  ...
  var calculateImageSource = function(image) {
    var slide =
      image.privacy.toUpperCase() == "PRIVATE"
        ? sprintf("%s%s", image.slide, image.author)
        : image.slide;
    return sprintf(
      "/proxyImageUrl/%s?source=%s",
      urlEncodeSlideName(slide),
      encodeURIComponent(image.source.trim())
    );
  };
  var calculateTextBounds = function(text) {
    text.bounds = [
      ...
      text.y,
      text.x + text.width,
      text.y + text.runs.length * text.size * 1.25
    ];
  };
  ...
  var prerenderImage = function(image) {
    ...
    var borderW = canvas.width * 0.1;
    var borderH = canvas.height * 0.1;
  ...
  var prerenderText = function(text) {
    ...
    var newline = /\n/;
    if (!text.width) {
      text.width = Math.max.apply(
        Math,
        text.text.split(newline).map(function(subtext) {
    ...
    function generateTextFont(text) {
      var font = text.font;
      if (text.weight == "bold") font = font + " bold";
      if (text.style == "italic") font = font + " italic";
    ...
    $.each(text.runs, function(ri, run) {
      ...
      if (text.decoration == "underline") underline();
    });
    incorporateBoardBounds(text.bounds);
  };
  var render = function(content) {
    if (content) {
      ...
      var fitMark,
        imagesRenderedMark,
        highlightersRenderedMark,
        textsRenderedMark,
        richTextsRenderedMark,
        inksRenderedMark,
        renderDecoratorsMark;
      try {
        var viewBounds = [
          viewboxX,
          viewboxY,
  ...
  var IncludeView = (function() {
    var fitToRequested = function(incX, incY, incW, incH) {
      //Include at least this much content in your view
      ...
      var constrained = Zoom.constrainRequestedViewbox({
        width: w,
        height: h,
        x: x,
        y: y
  ...
  var strokeCollected = function(spoints) {
    if (spoints.length > 0) {
      ...
      for (var p = 0; p < points.length; p += 3) {
        ...
        scaledPoints = scaledPoints.concat([
          worldPos.x,
          worldPos.y,
          points[p + 2]
        ]);
      ...
      ink.checksum = ink.points.reduce(function(a, b) {
        return a + b;
      }, 0);
  ...
  var transformReceived = function(transform) {
    ...
    var transformBounds = (function() {
      ...
      var incBoardBounds = function() {
        ...
        if (
          thisBounds[0] != undefined &&
          thisBounds[1] != undefined &&
          thisBounds[2] != undefined &&
          thisBounds[3] != undefined
        ) {
  ...
  var drawImage = function(image) {
    try {
      if (image.canvas != undefined) {
        ...
        var borderW = sBounds.screenWidth * 0.1;
        var borderH = sBounds.screenHeight * 0.1;
        boardContext.drawImage(
          image.canvas,
          sBounds.screenPos.x - borderW / 2,
          sBounds.screenPos.y - borderH / 2,
          sBounds.screenWidth + borderW,
          sBounds.screenHeight + borderH
        );
      }
    } catch (e) {
  ...
  var Modes = (function(thisBoard) {
    var removeActiveMode = function() {
      ...
      $(".activeMode")
        .addClass("inactiveMode")
        .removeClass("activeMode");
    };
    var setActiveMode = function(toolsSelector, headerSelector) {
      removeActiveMode();
      $(toolsSelector)
        .addClass("activeMode")
        .removeClass("inactiveMode");
      $(".activeTool")
        .removeClass("activeTool")
        .addClass("inactiveTool");
      $(headerSelector)
        .addClass("activeTool")
        .removeClass("inactiveTool");
    ...
    return {
      ...
      text: (function() {
        ...
        var fontFamilySelector,
          ...
          fontBoldSelector,
          fontItalicSelector,
          fontUnderlineSelector,
          justifySelector,
          presetFitToText,
          presetRunToEdge,
          presetNarrow,
          presetWiden,
          presetCenterOnScreen,
          presetFullscreen;
        ...
        $(function() {
          ...
          var fontFamilyOptionTemplate = fontFamilySelector
            .find(".fontFamilyOption")
            .clone();
          var fontSizeOptionTemplate = fontSizeSelector
            .find(".fontSizeOption")
            .clone();
          var fontColorOptionTemplate = fontColorSelector
            .find(".fontColorOption")
            .clone();
          Fonts.getAllFamilies().map(function(family) {
            fontFamilySelector.append(
              fontFamilyOptionTemplate
                .clone()
                .attr("value", family)
                .text(family)
            );
          ...
          Colors.getAllNamedColors().map(function(color) {
            fontColorSelector.append(
              fontColorOptionTemplate
                .clone()
                .attr("value", color.rgb)
                .text(color.name)
            );
          });
            ...
            return function() {
              _.each(boardContent.multiWordTexts, function(t) {
                if (t.doc.isActive) {
                  ...
                  selRange.setFormatting(
                    prop,
                    selRange.getFormatting()[prop] !== true
                  );
        ...
        return {
          ...
          editorFor: function(t) {
            ...
            if (!editor.doc) {
              ...
              editor.doc = carota.editor.create(
                $("<div />", { id: sprintf("t_%s", t.identity) }).appendTo(
                  textInputHost
                )[0],
                canvasElement[0],
                isAuthor
              ...
              if (isAuthor) {
                ...
                editor.doc.selectionChanged(function(formatReport) {
                  ...
                  fontItalicSelector.toggleClass(
                    "active",
                    format.italic == true
                  );
                  fontUnderlineSelector.toggleClass(
                    "active",
                    format.underline == true
                  );
                  fontSizeSelector.val(
                    format.size || carota.runs.defaultFormatting.size
                  );
                  fontFamilySelector.val(
                    format.font || carota.runs.defaultFormatting.font
                  );
                  fontColorSelector.val(
                    format.color || carota.runs.defaultFormatting.color
                  );
                  justifySelector.val(
                    format.align || carota.runs.defaultFormatting.align
                  );
                });
              }
            }
          ...
          activate: function() {
            ...
            var up = function(x, y, z, worldPos) {
              ...
              var selectedTexts = _.values(boardContent.multiWordTexts).filter(
                function(text) {
              ...
              if (selectedTexts.length > 0) {
                ...
                var relativePos = {
                  x: worldPos.x - editor.position.x,
                  y: worldPos.y - editor.position.y
                };
      ...
      image: (function() {
        ...
        var updateImageEditor = function() {
          if (
            ...
            currentImage.type == "imageDefinition"
          ) {
            ...
            if ("fileUpload" in currentImage) {
              ...
              $.map(imageSizeChoiceSelector.find(".imageSizeChoice"), function(
                elem
              ) {
                if (
                  "thumbnailSize" in currentImage &&
                  currentImage.thumbnailSize.name == $(elem).text()
                ) {
              ...
              reader.onload = function(e) {
                ...
                imageUploadThumbnail[0]
                  .getContext("2d")
                  .clearRect(
                    0,
                    0,
                    imageUploadThumbnail.width(),
                    imageUploadThumbnail.height()
                  );
                var img = new Image();
                img.onload = function(e) {
                  var resizedDimensions = currentImage.thumbnailSize.func(
                    img.width,
                    img.height
                  );
          ...
          if (
            ...
            $.ajax({
              ...
              success: function(e) {
                ...
                var imageStanza = {
                  ...
                  tag:
                    ...
                    '","id":"' +
                    newIdentity +
                    '","isBackground":false,"zIndex":0,"timestamp":-1}',
                ...
                updateTracking(newIdentity, function() {
                  ...
                  var newW = Math.max(
                    imageStanza.x + imageStanza.width,
                    viewboxWidth
                  );
                  var newH = Math.max(
                    imageStanza.y + imageStanza.height,
                    viewboxHeight
                  );
              ...
              error: function(e) {
                resetImageUpload();
                alert(
                  "Upload failed.  This image cannot be processed, either because of image protocol issues or because it exceeds the maximum image size."
                );
                WorkQueue.gracefullyResume();
              },
        ...
        $(function() {
          if (!hasInitialized) {
            ...
            if (imageFileChoice[0] != undefined) {
              imageFileChoice[0].addEventListener(
                "change",
                function(e) {
                  if (
                    "type" in currentImage &&
            ...
            var imageSizeOptionTemplate = imageSizeChoiceSelector
              .find(".imageSizeChoice")
              .clone();
            imageSizeChoiceSelector.empty();
            imageSizeChoices.map(function(isc) {
              var thisChoice = imageSizeOptionTemplate
                .clone()
                .text(isc.name)
                .on("click", function() {
                  if (
      ...
      select: (function() {
          ...
          if (sel) {
            ...
            var shouldShowButtons = function() {
              if ("images" in sel && _.size(sel.images) > 0) {
                ...
                "multiWordTexts" in sel &&
                _.size(sel.multiWordTexts) > 0
              ) {
                return true;
              } else {
                return false;
            ...
            if (Modes.currentMode == Modes.select) {
              ...
              _.forEach(
                ["images", "texts", "inks", "highlighters", "multiWordTexts"],
                function(category) {
                  if (category in sel) {
          ...
          _.forEach(
            ...
            function(catName) {
              ...
              if (
                ...
                Modes.select &&
                Modes.select.selected &&
                selCatName in Modes.select.selected
              ) {
                var cat = Modes.select.selected[selCatName];
                _.forEach(cat, function(i) {
                  if (
                    cat &&
                    boardCatName in boardContent &&
                    i.identity in boardContent[boardCatName]
                  ) {
                    ...
                    if (selCatName == "inks") {
                      if (
                        ...
                        i.identity in cat &&
                        cat[i.identity].isHighlighter == true
                      ) {
                        delete cat[i.identity];
                      } else if (
                        boardCatName == "inks" &&
                        i.identity in cat &&
                        cat[i.identity].isHighlighter == false
                      ) {
                        delete cat[i.identity];
                      }
                    } else {
        ...
        var banContentFunction = function() {
          if (Modes.select.selected != undefined && isAdministeringContent) {
            ...
            banContent(
              ...
              _.uniq(
                _.map(s.inks, function(e) {
                  return e.identity;
                })
              ),
              _.uniq(
                _.map(s.texts, function(e) {
                  return e.identity;
                })
              ),
              _.uniq(
                _.map(s.multiWordTexts, function(e) {
                  return e.identity;
                })
              ),
              _.uniq(
                _.map(s.images, function(e) {
                  return e.identity;
                })
              )
            );
        ...
        var administerContentFunction = function() {
          isAdministeringContent = Conversations.shouldModifyConversation()
            ? !isAdministeringContent
            : false;
          if (isAdministeringContent) {
            $("#administerContent").removeClass("disabledButton");
          } else {
        ...
        Progress.onBoardContentChanged[
          "ModesSelect"
        ] = updateSelectionWhenBoardChanges;
        Progress.onViewboxChanged[
          "ModesSelect"
        ] = updateSelectionWhenBoardChanges;
        ...
        Progress.conversationDetailsReceived["ModesSelect"] = function(
          conversation
        ) {
          if (
            isAdministeringContent &&
          ...
          activate: function() {
            ...
            $("#delete").bind("click", function() {
              if (Modes.select.selected != undefined) {
                ...
                if ("images" in Modes.select.selected) {
                  deleteTransform.imageIds = _.keys(
                    Modes.select.selected.images
                  );
                }
                if ("multiWordTexts" in Modes.select.selected) {
                  deleteTransform.multiWordTextIds = _.keys(
            ...
            $("#resize").bind("click", function() {
              ...
              if (items.length > 0) {
                ...
                var y1 = Math.min.apply(
                  Math,
                  items.map(function(item) {
                    return item.bounds[1];
                  })
                );
                var x2 = Math.max.apply(
                  Math,
                  items.map(function(item) {
                    return item.bounds[2];
                  })
                );
                var y2 = Math.max.apply(
                  Math,
                  items.map(function(item) {
            ...
            var down = function(x, y, z, worldPos, modifiers) {
              ...
              if (
                ...
                dragging = _.some(
                  ["images", "texts", "inks", "multiWordTexts"],
                  isDragHandle
                );
              }
            ...
            var up = function(x, y, z, worldPos, modifiers) {
              ...
              if (dragging) {
                ...
                _.forEach(Modes.select.selected.multiWordTexts, function(
                  image
                ) {
                  totalBounds.x = Math.min(image.bounds[0]);
                  totalBounds.y = Math.min(image.bounds[1]);
                ...
                resized.multiWordTextIds = _.keys(
                  Modes.select.selected.multiWordTexts
                );
                ...
                var selectionRect = rectFromTwoPoints(
                  marqueeWorldOrigin,
                  worldPos
                );
                var selectionBounds = [
                ...
                var intersectCategory = function(category) {
                  $.each(boardContent[category], function(i, item) {
                    ...
                    var b = item.bounds;
                    var selectionThreshold = Math.abs(
                      overlapThreshold * ((b[2] - b[0]) * (b[3] - b[1]))
                    );
                ...
                var status = sprintf(
                  "Selected %s images, %s texts, %s inks, %s rich texts ",
              ...
              Progress.call("onSelectionChanged", [Modes.select.selected]);
              marquee.css({ width: 0, height: 0 }).hide();
            };
            dragging = false;
            resizing = false;
            registerPositionHandlers(canvasElement, down, move, up);
      ...
      zoom: {
        ...
        activate: function() {
          ...
          var move = function(x, y, z, worldPos) {
            ...
            updateMarquee(
              marquee,
              { x: constrainedRect.right, y: constrainedRect.bottom },
              { x: constrainedRect.left, y: constrainedRect.top }
            );
          };
          var up = function(x, y, z, worldPos) {
            WorkQueue.gracefullyResume();
            marquee.hide();
            var currentPoint = {
              x: contentOffsetX + worldPos.x,
              y: contentOffsetY + worldPos.y
            };
            var startingWorldPoint = {
              x: contentOffsetX + startWorldPos.x,
              y: contentOffsetY + startWorldPos.y
            };
      ...
      feedback: (function() {
        ...
        Progress.conversationDetailsReceived[
          "respectNewPermissions"
        ] = applyStateStyling;
        return {
          name: "feedback",
          activate: function() {
      ...
      draw: (function() {
        ...
        return {
          name: "draw",
          brushes: _.map(originalBrushes, function(i) {
            return _.clone(i);
          }),
          activate: function() {
            ...
            drawAdvancedTools = function(brush) {
              ...
              var hlButton = $("#setPenToHighlighter")
                .unbind("click")
                .on("click", function() {
              ...
              if (
                "isHighlighter" in currentBrush &&
                currentBrush.isHighlighter
              ) {
